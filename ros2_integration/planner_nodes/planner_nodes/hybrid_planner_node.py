"""
ROS2 node that ports the thesis Hybrid A*+corridor-RRT* planner into a
live planning node. This is the thesis' novel method.

Subscribes:
    /map               (nav_msgs/OccupancyGrid) - the environment map
    /goal_pose_hybrid  (geometry_msgs/PoseStamped) - target pose for Hybrid planning

Uses TF2 to get the robot's current pose (map -> base_link).

Publishes:
    /planned_path_hybrid (nav_msgs/Path) - the planned path, viewable in RViz

Uses its own topic pair (/goal_pose_hybrid, /planned_path_hybrid) so all
three planner nodes (A*, RRT*, Hybrid) can run side by side for comparison.
...
class HybridPlannerNode(Node):
    def __init__(self):
        ...
        self.goal_sub = self.create_subscription(
            PoseStamped, '/goal_pose_hybrid', self.goal_callback, 10)

        self.path_pub = self.create_publisher(Path, '/planned_path_hybrid', 10)

        self.get_logger().info(
            'Hybrid planner node started, waiting for /map and /goal_pose_hybrid...')
    ...
    def goal_callback(self, msg: PoseStamped):
        ...
        self.get_logger().info(
            f'Hybrid planning from {start_cell} to {goal_cell} (grid cells)...')
    ...
    def publish_path(self, path_cells):
        ...
        self.get_logger().info(
            f'Published path with {len(path_msg.poses)} poses to /planned_path_hybrid')
